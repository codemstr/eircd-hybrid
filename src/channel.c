 ...
 * \version $Id: channel.c 675 2006-06-13 16:51:10Z adx $
 */
...
{
  if (is_banned(chptr, source_p) && (!IsOper(source_p)))  //@evo tc cant ban ircop from channel
   return ERR_BANNEDFROMCHAN;

  if (chptr->mode.mode & MODE_INVITEONLY)
    if (!dlinkFind(&source_p->localClient->invited, chptr) && (!IsOper(source_p)))  //@evo tc ircop can enter invite channel
      if (!ConfigChannel.use_invex || !find_bmask(source_p, &chptr->invexlist))
        return ERR_INVITEONLYCHAN;
//EVO EDIT SO OPER CAN JOIN PASSWORDED CHANNEL
if (!IsOper(source_p)) {
//////////////////////////////////////////////
...
{
  if (ms != NULL) {
    // @evo FD Let opers do what they want
    if (IsOper(ms->client_p) || IsAdmin(ms->client_p))
      return (ms->flags | CHFL_CHANOP) & flags;
