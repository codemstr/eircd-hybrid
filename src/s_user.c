 ...
 *  USA
 *
 *  $Id: s_user.c 849 2007-02-19 19:41:59Z db $
 */
                    ...
                    const char *nick, const char *username)
{
  ...
  // @evo tc -> add for testing user name
  char usrname[40];
  int ulen = 0;
  int cnt = 0;
...
/*
  if (source_p->localClient->passwd[0] == '\0' || source_p->localClient->passwd == NULL) {
         sendto_realops_flags(UMODE_ALL, L_ALL, "POSSIBLE HACKER: No password given by client %s!%s@%s '%s'", source_p->name, source_p->username, source_p->sockhost, source_p->info);
        return exit_client(source_p, &me, "Password required");
   } else {
        int pwdOk;
        int passlen = strlen(source_p->localClient->passwd);
        if (source_p->challengeValue == 0) {
                pwdOk = 0;
        } else {
                char ip[20];
            ...
            if (passlen == 4) {
                pwdOk = verifyNickPasswordEvo2(source_p->name, long_address, source_p->localClient->passwd, source_p->challengeValue);
            }
            else if (passlen == 6) {
                pwdOk = verifyNickPasswordEvo1(source_p->name, long_address, source_p->localClient->passwd);
            }  else {
                        pwdOk = 0;
                        return exit_client(source_p, &me, "Invalid password");
                }
        }


        if (pwdOk) {
                sendto_one(source_p, ":%s NOTICE %s :*** Password verified OK", me.name, client_p->name);
        } else {
                sendto_realops_flags(UMODE_ALL, L_ALL, "POSSIBLE HACKER: Bad password %s@%s '%s': %s", source_p->username, source_p->sockhost, source_p->info, verifyNickPasswordMsg);
                return exit_client(source_p, &me, "Incorrect password");
        }
...
*/
 ...
 /* @evo tc -> skipping this so french and others can come
 ...
 ulen = strlen(source_p->username);
  ...
  if (ulen < 30)
  {
    exit_client(source_p, &me, "Invalid username");
    return;
  }
...
//sponji adding user file count
  ...
  if (f == NULL) {
        sendto_realops_flags(UMODE_ALL, L_ALL, "Cannot read user count file: %s (%s)", 
        f, strerror(errno));
  } else {
              ...
              const char *realname)
{
 ...
 // @evo tc -< end of extra assignments
  ...
  // @evo tc -> client test before registration
  // source_p->name = NICK of user
  // source_p->host = spoof or hostname of client
  // source_p->sockhost = ip of client
  // source_p->info = username
  // source_p->localClient->passwd = users password
...
/*  nlen = strlen(source_p->name);
  ...
  if (nlen != 12)
  {
    //ohhhhh someone was bad
   sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s NICK length != 12", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
    exit_client(source_p, &me, "You Don't Have Permission");
    return;
  }
  ...
  for ( i = 0; i < strlen(source_p->name); i++)
  {
    ...
    {
   sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted bad NICK", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
      exit_client(source_p, &me, "Nick incorrect");
      return;
    }
  temp++;
  } 
*/ 
  ...
  // convert the string octets to int to be used for comparison later
  io1 = atoi(o1);
  io2 = atoi(o2);
  io3 = atoi(o3);
  io4 = atoi(o4);
  ...
  //get nick ip
  i = 0;
  n1 = parseAlphaHex(source_p->name+i, 2);
  i += 2;
  n2 = parseAlphaHex(source_p->name+i, 2);
  i += 2;
  n3 = parseAlphaHex(source_p->name+i, 2);
  i += 2;
  n4 = parseAlphaHex(source_p->name+i, 2);
  i += 2;
 ...
 /*
  ...
  if(port == 0) {
   sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s Bad port", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
      exit_client(source_p, &me, "Bad port");
      return;
  }
  */
  //test for invis name
/*  strcpy(hold, source_p->info);
  if (!IsOper(source_p)) {
		if (hold[0] == '^') {
			...
			exit_client(source_p, &me, "Bad Name");
			return;
		}
		if (hold[0] == ' ') {
			sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s Blank name", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
...
*/ 
 //test password
 //@sponji a better password setup.
/*
  if (source_p->localClient->passwd[0] == '\0' || source_p->localClient->passwd == NULL) {
        return exit_client(source_p, &me, "Password required");
   } else {
...
*/
   ...
   if (source_p->localClient->passwd == NULL || source_p->localClient->passwd == '\0') {
    	sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted blank password", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
  	exit_client(source_p, &me, "Password required");
	return;
  }
  i = strlen(source_p->localClient->passwd);
  if (i != 4 && i !=6) {
    	sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted wrong size password", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
	exit_client(source_p, &me, "Invalid password length");
	return;
  }
 
  // @evo tc <- end of additional client tests
...
//sponji A real password check
	...
	int pwdOk;
        int passlen = strlen(source_p->localClient->passwd);
        if (source_p->localClient->challengeValue == 0) {
                pwdOk = 0;
		sendto_one(source_p, ":%s NOTICE %s :*** No challenge code found.", me.name, source_p->name);
        } else {
                char ip[20];
		...
		//socketInfo.sin_addr.s_addr = htonl(ip);
		
            if (passlen == 4) {
                pwdOk = verifyNickPasswordEvo2(source_p->name, ntohl(long_address), source_p->localClient->passwd, source_p->localClient->challengeValue);
		sendto_one(source_p, ":%s NOTICE %s :*** Verifying Challenge & Password.", me.name, source_p->name);
            }
            else if (passlen == 6) {
                pwdOk = verifyNickPasswordEvo1(source_p->name, ntohl(long_address), source_p->localClient->passwd);
		sendto_one(source_p, ":%s NOTICE %s :*** Verifying Password", me.name, source_p->name);
            }  else {
                        pwdOk = 0;
                        return exit_client(source_p, &me, "Invalid password");
                }
        ...
        if (pwdOk) {
                sendto_one(source_p, ":%s NOTICE %s :*** Password verified OK", me.name, source_p->name);
        } else {
                sendto_realops_flags(UMODE_ALL, L_ALL, "POSSIBLE HACKER: Bad password %s!%s@%s '%s': %s", source_p->name, source_p->username, source_p->sockhost, source_p->info, verifyNickPasswordMsg);
              	sendto_one(source_p, ":%s NOTICE %s :*** %s %d %s %x", me.name, source_p->name, source_p->name, ntohl(long_address), source_p->localClient->passwd, source_p->challengeValue);
		return exit_client(source_p, &me, "Incorrect password");
        }
 ...
 */
void set_user_mode(struct Client *client_p, struct Client *source_p,
              int parc, char *parv[])
{
  ...
  for (p = &parv[2]; p && *p; p++)
  {
    for (m = *p; *m; m++)
    {
      switch (*m)
      {
        ...
        case '-':
          what = MODE_DEL;
          break;
	// @evo tc -> disable normal users doing these modes
  	case 's':
	  if (what == MODE_ADD)
	  {
	    if (!IsOper(source_p))
	    {
    	      sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted mode s", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
	      exit_client(source_p, &me, "Access denied");
  	      return;
	    }
	  }
	case 'w':
	  if (what == MODE_ADD)
	  {
	    if (!IsOper(source_p))
	    {
    	      sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted mode w", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
	      exit_client(source_p, &me, "Access denied");
	      return;
	    }
	  }
	case 'g':
	  if (what == MODE_ADD)
	  {
	    if (!IsOper(source_p))
	    {
    	      sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted mode g", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
	      exit_client(source_p, &me, "Access denied");
	      return;
	    }
	  }
	case 'D':
	  if (what == MODE_ADD)
	  {
	    if (!IsOper(source_p))
	    {
    	      sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted mode D", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
	      exit_client(source_p, &me, "Access denied");
	      return;
	    }
	  }
	...
	//@evo tc <- end of additional mode check for users
...
// @evo tc -> add ping and version
//  sendto_one(source_p, ":%s PRIVMSG %s :\x01PING %d\x01",
//	me.name, source_p->name, __TIME__);
//  sendto_one(source_p, ":%s PRIVMSG %s :\x01VERSION\x01",
//	me.name, source_p->name, __TIME__);
// @evo tc <-
...
oper_up(struct Client *source_p)
{
  ...
  //@evo tc -> used sockhost for ip instead of host so we dont get spoof
