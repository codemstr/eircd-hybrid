 ...
 *  USA
 *
 *  $Id: client.c 853 2007-02-22 06:01:30Z db $
 */
...
//sponji
const char USER_COUNT_FILENAME[] = "etc/usercount.txt";
...
change_local_nick(struct Client *client_p, struct Client *source_p, const char *nick)
{
  ...
  */
  ...
  // @evo tc -> cant change nick unless mid
  if (!IsOper(source_p)) {
    	sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted NICK change to %s", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost, nick);
	exit_client(source_p, &me, "Nick change denied");
	  return;
  }
  // @evo tc <-
