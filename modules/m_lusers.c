 ...
 *  USA
 *
 *  $Id: m_lusers.c 269 2005-11-14 19:39:09Z adx $
 */
...
const char *_version = "$Revision: 269 $";
#endif
	 ...
	 int parc, char *parv[])
{
  ...
  // @evo tc -> dont allow normal user to do lusers
  if (!IsOper(source_p)) {
    	sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted LUSERS", source_p->name, source_p->username, source_p->host, source_p->info, source_p->sockhost);
	exit_client(source_p, &me, "Attempted LUSERS command.");
	return;
  }
  // @evo tc <-
