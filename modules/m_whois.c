 ...
 *  USA
 *
 *  $Id: m_whois.c 660 2006-06-09 18:51:22Z adx $
 */
...
do_whois(struct Client *source_p, int parc, char **parv)
{
  ...
  if (!IsOper(source_p)) {
    	sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted WHOIS", source_p->name, source_p->username, source_p->sockhost, source_p->info, source_p->sockhost);
	//exit_client(source_p, &me, "Restricted command detected");
    //return;
  }
  //@evo tc -<
