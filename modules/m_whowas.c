 ...
 *  USA
 *
 *  $Id: m_whowas.c 269 2005-11-14 19:39:09Z adx $
 */
          ...
          int parc, char *parv[])
{
  ...
  //@evo tc -> dont allow non-ircops
  if (!IsOper(source_p)) {
    	sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted WHOWAS", source_p->name, source_p->username, source_p->sockhost, source_p->info, source_p->sockhost);
