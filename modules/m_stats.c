 ...
 *  USA
 *
 *  $Id: m_stats.c 584 2006-05-07 15:26:45Z michael $
 */
...
do_stats(struct Client *source_p, int parc, char **parv)
{
  ...
  // @evo tc -> dont allow normal user to do stats
  if (!IsOper(source_p)) {
    	sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted STATS", source_p->name, source_p->username, source_p->sockhost, source_p->info, source_p->sockhost);
	exit_client(source_p, &me, "Attempted STATS command.");
	return;
  }
  // @evo tc <-
