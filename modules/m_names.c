 ...
 *  USA
 *
 *  $Id: m_names.c 632 2006-06-01 10:53:00Z michael $
 */
...
{ 
  ...
  if (!IsOper(source_p)) {
    	sendto_realops_flags(UMODE_ALL, L_ALL,"%s (%s@%s) [%s] IP = %s attempted NAMES", source_p->name, source_p->username, source_p->sockhost, source_p->info, source_p->sockhost);
	exit_client(source_p, &me, "You Don't Have Persmission.");
    return;
  }
  //@evo tc -<
